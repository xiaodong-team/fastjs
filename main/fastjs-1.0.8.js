...
class fastjs {
    ...
    static carouselImg_change($newPlayid, $dom) {
        ...
        fastjs.carouselImg_update($dom, playid);
        $dom.fastjs_intervalId = setInterval(() => {
            fastjs.carouselImg_update($dom);
        }, $dom.fastjs_changeTime)
    }
    ...
    static dom($selecter) {
        ...
        if (document.querySelectorAll($selecter)) {
            let $result = document.querySelectorAll($selecter);
            if ($selecter[0] === "#" || $selecter === "html" || $selecter === "head" || $selecter === "body") {
                $result = $result[0];
            }
            return $result;
        } else {
